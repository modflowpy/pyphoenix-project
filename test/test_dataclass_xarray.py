"""Tests for the generic dataclass<->xarray conversion layer
(flopy4/dataclass_xarray.py, flopy4/protocols.py, flopy4/mixins.py).

Uses synthetic dataclasses to validate the conversion functions in
...
class Leaf(DatasetConvertibleMixin):
    ...
    name: str = "leaf"
    value: float = 1.0
    # Deliberately not named "dims"/"parent"/"_parent" -- flopy4.dataclass_xarray
...
def test_leaf_scalar_and_array_round_trip():
    ...
    rebuilt = dataset_to_dataclass(Leaf, ds)
...
def test_node_dict_children_not_reconstructed():
    ...
    field from the tree alone -- see flopy4/dataclass_xarray.py's module docstring.
    """
...
def test_dis_leaf_round_trip_real_mf6_class():
    """Dis has no dataclass-typed children when ncf is unset, so it's a real
    (not synthetic) example of a DatasetConvertible-shaped leaf class."""
    dis = Dis(nlay=2, nrow=3, ncol=4)
    ds = dataclass_to_dataset(dis)
    ...
    rebuilt = dataset_to_dataclass(Dis, ds)
