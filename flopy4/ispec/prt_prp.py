...
"""exit solve method""",
        description =
"""the method for iterative solution of particle exit location and time
...
"""exit solve tolerance""",
        description =
"""the convergence tolerance for iterative solution of particle exit
location and time in the generalized Pollock's method.  A value of
...
best balance between accuracy and runtime is problem-dependent.""",
    ...
    local_z = MFKeyword(
        ...
        optional = True,
        longname =
"""whether to use local z coordinates""",
        description =
"""indicates that ``zrpt'' defines the local z coordinate of the release
point within the cell, with value of 0 at the bottom and 1 at the top
of the cell.  If the cell is partially saturated at release time, the
...
head in the cell) rather than the top defined by the user.""",
    ...
    extend_tracking = MFKeyword(
        ...
        optional = True,
        longname =
"""whether to extend tracking beyond the end of the simulation""",
        description =
"""indicates that particles should be tracked beyond the end of the
simulation's final time step (using that time step's flows) until
particles terminate or reach a specified stop time.  By default,
...
"""stop time""",
        description =
"""real value defining the maximum simulation time to which particles in
the package can be tracked.  Particles that have not terminated
earlier due to another termination condition will terminate when
simulation time STOPTIME is reached.  If the last stress period in the
simulation consists of more than one time step, particles will not be
tracked past the ending time of the last stress period, regardless of
STOPTIME.  If the last stress period in the simulation consists of a
single time step, it is assumed to be a steady-state stress period,
and its ending time will not limit the simulation time to which
...
"""stop travel time""",
        description =
"""real value defining the maximum travel time over which particles in
the model can be tracked.  Particles that have not terminated earlier
due to another termination condition will terminate when their travel
time reaches STOPTRAVELTIME.  If the last stress period in the
simulation consists of more than one time step, particles will not be
tracked past the ending time of the last stress period, regardless of
STOPTRAVELTIME.  If the last stress period in the simulation consists
of a single time step, it is assumed to be a steady-state stress
period, and its ending time will not limit the travel time over which
particles can be tracked.  If STOPTIME and STOPTRAVELTIME are both
provided, particles will be stopped if either is reached.""",
    )
...
"""stop at weak sink""",
        description =
"""is a text keyword to indicate that a particle is to terminate when it
...
"""stop zone number""",
        description =
"""integer value defining the stop zone number.  If cells have been
assigned IZONE values in the GRIDDATA block, a particle terminates if
it enters a cell whose IZONE value matches ISTOPZONE.  An ISTOPZONE
...
"""drape""",
        description =
"""is a text keyword to indicate that if a particle's release point is in
a cell that happens to be inactive at release time, the particle is to
be moved to the topmost active cell below it, if any. By default, a
...
"""is the number of particle release points.""",
    ...
    irptno = MFInteger(
        ...
        optional = False,
        longname =
"""PRP id number for release point""",
        description =
"""integer value that defines the PRP release point number associated
with the specified PACKAGEDATA data on the line. IRPTNO must be
greater than zero and less than or equal to NRELEASEPTS.  The program
...
must lie within the cell that is identified by the specified cellid.""",
    ...
    zrpt = MFDouble(
        ...
        optional = False,
        longname =
"""z coordinate of release point""",
        description =
"""real value that defines the z coordinate of the release point in model
coordinates or, if the LOCAL_Z option is active, in local cell
coordinates.  The (x, y, z) location specified for the release point
must lie within the cell that is identified by the specified cellid.""",
    )
    boundname = MFString(
...
"""release point name""",
        description =
"""name of the particle release point. BOUNDNAME is an ASCII character
...
"""""",
        description =
"""release particles after the specified fraction of the time step has
elapsed. If FRACTION is not set, particles are released at the start
of the specified time step(s). FRACTION must be a single value when
