...
class Dis(DisBase):
    ...
    # TODO: should be emitted by codegen from ncf_filerecord in gwe-dis.dfn.
    ...
    ncol: int = dim(
        block="dimensions", coord="col", scope="gwe", default=2, longname="number of columns"
    )
    nrow: int = dim(
        block="dimensions", coord="row", scope="gwe", default=2, longname="number of rows"
    )
    ...
    nodes: int = dim(coord="node", scope="gwe", init=False)
    ncpl: int = dim(coord="c", scope="gwe", init=False)
    nvert: int = dim(coord="vert", scope="gwe", init=False)
