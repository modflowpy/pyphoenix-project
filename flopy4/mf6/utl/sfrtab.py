...
class Sfrtab(Package):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        _table_f: list[tuple] = []
        for _col in self.__table_schema__:
            ...
            if _role == "cellid":
                _table_f.append((_col["name"], np.int64, (ncelldim,)))
            elif _role == "feature_id":
                _table_f.append((_col["name"], np.int64))
            elif _role == "boundname":
                if getattr(self, "boundnames", False):
                    _table_f.append((_col["name"], np.object_))
            else:
                _table_f.append((_col["name"], _dt))
        if "table" == "packagedata":
            for _i in range(naux):
                _table_f.append((f"aux{_i}", np.object_))
        self.table_dtype = np.dtype(_table_f)
        _raw_table = getattr(self, "table", None)
        if _raw_table is not None:
            self.table = self._coerce_to_recarray(_raw_table, self.table_dtype)
        if getattr(self, "table", None) is not None and getattr(self, "ntables", 0) == 0:
            object.__setattr__(self, "ntables", len(getattr(self, "table")))
